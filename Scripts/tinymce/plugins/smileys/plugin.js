    ...
    function getHtml() {
        ...
        tinymce.each(fullSmileysList, function (row) {
            ...
            tinymce.each(row, function (icon) {
                smileysHtml += '<td><a href="#" data-mce-url="' + icon.url + '" contextmenu="' + icon.shortcut + '" tabindex="-1" title="' + icon.title + '"><img src="' +
        ...
        if (!editor.settings.auto_convert_smileys) {
            ...
            if (node) {
                each(concatArray(fullSmileysList), function (smiley) {
                  //  replaceAllMatches(smiley); // disabled by Fahmi in order to be able to post the emoticon symbol to facebook and not the image
                });
            }
        }
    });
    ...
    editor.addButton('smileys', {
        ...
        panel: {
            ...
            onclick: function (e) {
                ...
                if (linkElm) {
                    //Below line was the default : updated by Fahmi in order to Publish Smilies on Facebook
                    //editor.insertContent('<img src="' + linkElm.getAttribute('data-mce-url') + '" title="' + linkElm.getAttribute('title') + '" />');
                    editor.insertContent(linkElm.getAttribute('contextmenu'));
                    this.hide();
                }
            }
        },
